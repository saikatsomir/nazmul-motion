...
import { MoreVertical, CalendarDays, User, Wallet } from 'lucide-react'; // Consolidated all icons to lucide-react

// Configuration Constants
const API_BASE_URL = 'https://nazmulmotion.com/api/payment';
const DARK_BG = '#0a0612'; // Main background color (Deep Black)
...
const ACCENT_COLOR_BASE = '#4107a4'; // Base accent color for background glow
const PENDING_HIGHLIGHT = '#ffc107'; // Amber/Yellow for Pending status
...
export const Pending = () => {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        // Fetch data from the new pending endpoint
        ...
        setError(
          'Failed to load Pending student data. Please check the backend server status.'
        );
        setLoading(false);
      }
    };
  ...
  const handleActionClick = (studentId) => {
    ...
    console.log(`Mock: Showing pending options for Student ID: ${studentId}`);
  };
    ...
    >
      <div className="flex justify-between items-start mb-3">
        <h3 className="text-lg font-bold" style={{ color: PRIMARY_TEXT }}>
          <User className="inline mr-2" style={{ color: PENDING_HIGHLIGHT }} />
          {student.name}
        </h3>
        <button
  ...
  return (
    <div
      ...
      style={{ backgroundColor: DARK_BG, color: PRIMARY_TEXT }}
    >
      {/* Background Gradient Blur Effects (Retained the original purple color) */}
      <div
      ...
      <div
        ...
        style={{ backgroundColor: PENDING_HIGHLIGHT }}
      ></div>
